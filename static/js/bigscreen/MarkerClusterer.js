...
(function(){
    ...
    var MarkerClusterer =  
        ...
        BMapLib.MarkerClusterer = function(map, options){
            ...
            this._markers = [];
            this._clusters = [];
            this._isMarkerClickable = true;
            
            var opts = options || {};
            this._gridSize = opts["gridSize"] || 60;
            this._maxZoom = opts["maxZoom"] || 15;//聚合级别//点击跳转基本大于这个级别即可以完成
    ...
    MarkerClusterer.prototype._createClusters = function(){
		...
		{            
			...
			else
			{
				...
				for (var f = 0; f < len; f++)
				{
                    lat += cluster._markers[f].getPosition().lat;
                    lng += cluster._markers[f].getPosition().lng;

                    var tempLabel = cluster._markers[f].getLabel();
					cluster._markers[f].getMap() && cluster._map.removeOverlay(cluster._markers[f]);
                    cluster._markers[f].setLabel(tempLabel);
				}
                cluster._center = new BMap.Point(lng / len, lat / len);
    ...
    MarkerClusterer.prototype.getMarkerClickable = function() {
        return this._isMarkerClickable;
    };

    MarkerClusterer.prototype.setMarkerClickable = function(clickable) {
        this._isMarkerClickable = clickable;
    };
    ...
    Cluster.prototype.addMarker = function(marker){
        ...
        if(len < this._minClusterSize ){
            ...
            for (var i = 0; i < len; i++) {
                var tempLabel = this._markers[i].getLabel();
                this._markers[i].getMap() && this._map.removeOverlay(this._markers[i]);
                this._markers[i].setLabel(tempLabel);
            }
    ...
    Cluster.prototype.updateClusterMarker = function () {
        ...
        var clustererM = this._markerClusterer;
        this._clusterMarker.addEventListener("click", function(event){
            if (!clustererM.getMarkerClickable()) {
                return;
            }
